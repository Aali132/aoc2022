...
pruneGeode ss = filter (\(_,(_,_,_,rob3),_) -> rob3 >= gc) ss
...
evalBlueprint bp = countGeodes (head ((iterate (expandStates bp) [start]) !! 22))
...
aoc :: [String] -> String
aoc f = show(sum (bps))
  where bps = map (\(a,b) -> a * b) (zip (map evalBlueprint (map parse f)) [1..])
